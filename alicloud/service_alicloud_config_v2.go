...
func (s *ConfigServiceV2) DescribeConfigReportTemplate(id string) (object map[string]interface{}, err error) {
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"ReportTemplateNotExist"}) {
			return object, WrapErrorf(NotFoundErr("ReportTemplate", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}

	v, err := jsonpath.Get("$.ReportTemplate", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.ReportTemplate", response)
	}
