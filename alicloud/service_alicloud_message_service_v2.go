...
// DescribeMessageServiceEventRule <<< Encapsulated get interface for MessageService EventRule.

func (s *MessageServiceServiceV2) DescribeMessageServiceEventRule(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"RESOURCE_NOT_FOUND"}) {
			return object, WrapErrorf(NotFoundErr("EventRule", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *MessageServiceServiceV2) MessageServiceEventRuleStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		v, err := jsonpath.Get(field, object)
		currentStatus := fmt.Sprint(v)
		if field == "$.Endpoint.EndpointValue" {
			e := jsonata.MustCompile("$split($.Endpoint.EndpointValue, \"/queues/\")[-1]")
			v, _ = e.Eval(object)
			currentStatus = fmt.Sprint(v)
		}
