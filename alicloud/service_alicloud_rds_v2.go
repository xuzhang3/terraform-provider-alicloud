...
// DescribeRdsCustomDiskAttachment <<< Encapsulated get interface for Rds CustomDiskAttachment.

func (s *RdsServiceV2) DescribeRdsCustomDiskAttachment(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	request = make(map[string]interface{})
	query = make(map[string]interface{})
	query["DiskIds"] = convertListToJsonString(expandSingletonToList(parts[0]))
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"InvalidUserAccount.NotSupported"}) {
			return object, WrapErrorf(NotFoundErr("CustomDiskAttachment", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
// DescribeRdsCustomDiskAttachment >>> Encapsulated.
