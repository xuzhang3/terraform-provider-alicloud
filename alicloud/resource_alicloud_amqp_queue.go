...
func resourceAliCloudAmqpQueueCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOkExists("maximum_priority"); ok {
	...
	d.SetId(fmt.Sprintf("%v:%v:%v", request["InstanceId"], request["VirtualHost"], request["QueueName"]))
...
func resourceAliCloudAmqpQueueRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("virtual_host_name", object["VHostName"])
	d.Set("queue_name", object["Name"])
