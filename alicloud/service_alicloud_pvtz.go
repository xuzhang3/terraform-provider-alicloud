...
func (s *PvtzService) DescribePvtzServiceStatus() (object map[string]interface{}, err error) {
	var response map[string]interface{}
	client := s.client
	...
	if err != nil {
		return object, WrapErrorf(err, DefaultErrorMsg, "PvtzService", action, AlibabaCloudSdkGoERROR)
	}

	v, err := jsonpath.Get("$", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, "PvtzService", "$", response)
	}
...
func (s *PvtzService) PvtzServiceStatusStateRefreshFunc(failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.DescribePvtzServiceStatus()
		if err != nil {
			if NotFoundError(err) {
				// Set this to nil as if we didn't find anything.
				return nil, "", nil
