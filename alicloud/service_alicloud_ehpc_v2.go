...
// DescribeEhpcQueue <<< Encapsulated get interface for Ehpc Queue.

func (s *EhpcServiceV2) DescribeEhpcQueue(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"ClusterNotFound", "InvalidParams"}) {
			return object, WrapErrorf(NotFoundErr("Queue", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}

	v, err := jsonpath.Get("$.Queue", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.Queue", response)
	}
