...
func TestAccAliCloudVpcNetworkAclAttachment_basic0(t *testing.T) {
	...
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, resourceAlicloudVpcNetworkAclAttachmentBasicDependence)
...
var resourceAlicloudVpcNetworkAclAttachmentMap = map[string]string{
	"status": CHECKSET,
}

func resourceAlicloudVpcNetworkAclAttachmentBasicDependence(name string) string {
	return fmt.Sprintf(`
	variable "name" {
  		default = "%s"
	}
...
func TestAccAliCloudVpcNetworkAclAttachment_basic2(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_vpc_network_acl_attachment.default"
	ra := resourceAttrInit(resourceId, resourceAlicloudVpcNetworkAclAttachmentMap2)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &VpcServiceV2{testAccProvider.Meta().(*connectivity.AliyunClient)}
	}, "DescribeVpcNetworkAclAttachment")
	rac := resourceAttrCheckInit(rc, ra)
	...
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tf-testAccVpcNetworkAclAttachment-name%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, resourceAlicloudVpcNetworkAclAttachmentBasicDependence2)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
...
var resourceAlicloudVpcNetworkAclAttachmentMap2 = map[string]string{
	"status": CHECKSET,
}

func resourceAlicloudVpcNetworkAclAttachmentBasicDependence2(name string) string {
	return fmt.Sprintf(`
	variable "name" {
  		default = "%s"
	}
	...
	resource "alicloud_vswitch" "default1" {
		count      = 22
  		vpc_id     = alicloud_vpc.default.id
  		cidr_block = cidrsubnet(alicloud_vpc.default.cidr_block, 8, count.index+2)
  		zone_id    = data.alicloud_zones.default.zones.1.id
	}
	...
	resource "alicloud_vpc_network_acl_attachment" "default1" {
      count			 = 22
	  network_acl_id = alicloud_network_acl.default.id
	  resource_id    = alicloud_vswitch.default1[count.index].id
	  resource_type  = "VSwitch"
	}
`, name)
}
...
func TestUnitAlicloudVpcNetworkAclAttachment(t *testing.T) {
	...
	dExisted, _ := schema.InternalMap(p["alicloud_vpc_network_acl_attachment"].Schema).Data(nil, nil)
	dInit.MarkNewResource()
	attributes := map[string]interface{}{
		"network_acl_id": "CreateVpcNetworkAclAttachmentValue",
		"resource_id":    "CreateVpcNetworkAclAttachmentValue",
		"resource_type":  "CreateVpcNetworkAclAttachmentValue",
	}
		...
		"NetworkAcls": map[string]interface{}{
			"NetworkAcl": []interface{}{
				map[string]interface{}{
					"NetworkAclId": "CreateVpcNetworkAclAttachmentValue",
					"Resources": map[string]interface{}{
						"Resource": []interface{}{
							map[string]interface{}{
								"ResourceId":   "CreateVpcNetworkAclAttachmentValue",
								"ResourceType": "CreateVpcNetworkAclAttachmentValue",
								"Status":       "BINDED",
							},
						},
					},
	...
	notFoundResponseMock := func(errorCode string) (map[string]interface{}, error) {
		return nil, GetNotFoundErrorFromString(GetNotFoundMessage("alicloud_vpc_network_acl_attachment", errorCode))
	}
	...
	for index, errorCode := range errorCodes {
		...
		default:
			assert.Nil(t, err)
			dCompare, _ := schema.InternalMap(p["alicloud_vpc_network_acl_attachment"].Schema).Data(dInit.State(), nil)
			for key, value := range attributes {
	...
	diff, err := newInstanceDiff("alicloud_vpc_network_acl_attachment", attributes, attributesDiff, dInit.State())
	if err != nil {
		t.Error(err)
	}
	dExisted, _ = schema.InternalMap(p["alicloud_vpc_network_acl_attachment"].Schema).Data(dInit.State(), diff)
	errorCodes = []string{"NonRetryableError", "Throttling", "nil", "{}"}
	for index, errorCode := range errorCodes {
		retryIndex := index - 1
		patches = gomonkey.ApplyMethod(reflect.TypeOf(&client.Client{}), "DoRequest", func(_ *client.Client, action *string, _ *string, _ *string, _ *string, _ *string, _ map[string]interface{}, _ map[string]interface{}, _ *util.RuntimeOptions) (map[string]interface{}, error) {
	...
	for index, errorCode := range errorCodes {
		retryIndex := index - 1
		patches := gomonkey.ApplyMethod(reflect.TypeOf(&client.Client{}), "DoRequest", func(_ *client.Client, action *string, _ *string, _ *string, _ *string, _ *string, _ map[string]interface{}, _ map[string]interface{}, _ *util.RuntimeOptions) (map[string]interface{}, error) {
			if *action == "UnassociateNetworkAcl" {
				switch errorCode {
				case "NonRetryableError":
					return failedResponseMock(errorCode)
				default:
