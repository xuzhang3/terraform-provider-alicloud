...
func resourceAliCloudResourceManagerMultiAccountDeliveryChannel() *schema.Resource {
	return &schema.Resource{
		Create: resourceAliCloudResourceManagerMultiAccountDeliveryChannelCreate,
		Read:   resourceAliCloudResourceManagerMultiAccountDeliveryChannelRead,
		Update: resourceAliCloudResourceManagerMultiAccountDeliveryChannelUpdate,
		Delete: resourceAliCloudResourceManagerMultiAccountDeliveryChannelDelete,
...
func resourceAliCloudResourceManagerMultiAccountDeliveryChannelCreate(d *schema.ResourceData, meta interface{}) error {
	...
	request["DeliveryChannelName"] = d.Get("multi_account_delivery_channel_name")
	dataList := make(map[string]interface{})

	if v := d.Get("delivery_channel_filter"); v != nil {
		accountScopes1, _ := jsonpath.Get("$[0].account_scopes", v)
		if accountScopes1 != nil && accountScopes1 != "" {
			dataList["AccountScopes"] = accountScopes1
		}
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alicloud_resource_manager_multi_account_delivery_channel", action, AlibabaCloudSdkGoERROR)
	}

	d.SetId(fmt.Sprint(response["DeliveryChannelId"]))

	return resourceAliCloudResourceManagerMultiAccountDeliveryChannelRead(d, meta)
}

func resourceAliCloudResourceManagerMultiAccountDeliveryChannelRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	resourceManagerServiceV2 := ResourceManagerServiceV2{client}

	objectRaw, err := resourceManagerServiceV2.DescribeResourceManagerMultiAccountDeliveryChannel(d.Id())
	if err != nil {
		if !d.IsNewResource() && NotFoundError(err) {
			log.Printf("[DEBUG] Resource alicloud_resource_manager_multi_account_delivery_channel DescribeResourceManagerMultiAccountDeliveryChannel Failed!!! %s", err)
			d.SetId("")
			return nil
		}
	...
	d.Set("multi_account_delivery_channel_name", objectRaw["DeliveryChannelName"])

	deliveryChannelFilterMaps := make([]map[string]interface{}, 0)
	deliveryChannelFilterMap := make(map[string]interface{})
	deliveryChannelFilterRaw := make(map[string]interface{})
	if objectRaw["DeliveryChannelFilter"] != nil {
		deliveryChannelFilterRaw = objectRaw["DeliveryChannelFilter"].(map[string]interface{})
	}
	if len(deliveryChannelFilterRaw) > 0 {

		accountScopesRaw := make([]interface{}, 0)
		if deliveryChannelFilterRaw["AccountScopes"] != nil {
			accountScopesRaw = convertToInterfaceArray(deliveryChannelFilterRaw["AccountScopes"])
		}

		deliveryChannelFilterMap["account_scopes"] = accountScopesRaw
		resourceTypesRaw := make([]interface{}, 0)
		if deliveryChannelFilterRaw["ResourceTypes"] != nil {
			resourceTypesRaw = convertToInterfaceArray(deliveryChannelFilterRaw["ResourceTypes"])
		}
...
func resourceAliCloudResourceManagerMultiAccountDeliveryChannelUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	var request map[string]interface{}
	var response map[string]interface{}
	...
	if d.HasChange("multi_account_delivery_channel_name") {
		update = true
	}
	request["DeliveryChannelName"] = d.Get("multi_account_delivery_channel_name")
	...
	if v := d.Get("delivery_channel_filter"); v != nil {
		accountScopes1, _ := jsonpath.Get("$[0].account_scopes", v)
		if accountScopes1 != nil && (d.HasChange("delivery_channel_filter.0.account_scopes") || accountScopes1 != "") {
			dataList["AccountScopes"] = accountScopes1
		}
		resourceTypes1, _ := jsonpath.Get("$[0].resource_types", v)
...
func resourceAliCloudResourceManagerMultiAccountDeliveryChannelDelete(d *schema.ResourceData, meta interface{}) error {
