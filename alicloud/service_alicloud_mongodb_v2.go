...
// DescribeMongodbNode <<< Encapsulated get interface for Mongodb Node.

func (s *MongodbServiceV2) DescribeMongodbNode(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"Node.NotFound"}) {
...
// DescribeMongodbNode >>> Encapsulated.
