...
func TestUnitAlicloudHavip(t *testing.T) {
	...
	responseMock := map[string]func(errorCode string) (map[string]interface{}, error){
		...
		"ReadDescribeHaVipsNotFound": func(errorCode string) (map[string]interface{}, error) {
	...
	t.Run("UpdateVpcHavIpAbnormal", func(t *testing.T) {
		diff := terraform.NewInstanceDiff()
		for _, key := range []string{"description", "havip_name"} {
			switch p["alicloud_havip"].Schema[key].Type {
		...
		resourceData1, _ := schema.InternalMap(p["alicloud_havip"].Schema).Data(nil, diff)
	...
	t.Run("UpdateVpcHavIpNormal", func(t *testing.T) {
		diff := terraform.NewInstanceDiff()
		for _, key := range []string{"description", "havip_name"} {
			switch p["alicloud_havip"].Schema[key].Type {
			case schema.TypeString:
	...
	//Read
	t.Run("ReadDescribeHaVipsNotFound", func(t *testing.T) {
		patcheDorequest := gomonkey.ApplyMethod(reflect.TypeOf(&client.Client{}), "DoRequest", func(_ *client.Client, _ *string, _ *string, _ *string, _ *string, _ *string, _ map[string]interface{}, _ map[string]interface{}, _ *util.RuntimeOptions) (map[string]interface{}, error) {
			NotFoundFlag := true
			noRetryFlag := false
			if NotFoundFlag {
				return responseMock["ReadDescribeHaVipsNotFound"]("")
			} else if noRetryFlag {
