...
// DescribeDdosBgpIp <<< Encapsulated get interface for DdosBgp Ip.

func (s *DdosBgpServiceV2) DescribeDdosBgpIp(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	request["Ip"] = parts[1]
	request["RegionId"] = client.RegionId
	request["PageNo"] = parts[1]
	request["PageSize"] = parts[1]
	...
	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("Ip", id), NotFoundMsg, response)
	}
...
// DescribeDdosBgpIp >>> Encapsulated.
