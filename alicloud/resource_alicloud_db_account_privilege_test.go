...
func TestAccAlicloudRdsDBAccountPrivilege_SQLServer(t *testing.T) {
	...
	name := fmt.Sprintf("tf-testAccDBAccountPrivilege_SQLServer_%d", rand)
	resourceId := "alicloud_db_account_privilege.default"
	var basicMap = map[string]string{
		"instance_id":  CHECKSET,
		"account_name": "tftestprivilege",
	...
	testAccCheck := rac.resourceAttrMapUpdateSet()
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, resourceDBAccountPrivilegeConfigDependenceForSQLServer)
...
func resourceDBAccountPrivilegeConfigDependenceForSQLServer(name string) string {
	return fmt.Sprintf(`
variable "name" {
	default = "%s"
}
