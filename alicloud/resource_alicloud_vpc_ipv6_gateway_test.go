...
func TestUnitAlicloudVPCIpv6Gateway(t *testing.T) {
	...
	t.Run("UpdateModifyIpv6GatewaySpecAbnormal", func(t *testing.T) {
		...
		for _, key := range []string{"spec"} {
			switch p["alicloud_vpc_ipv6_gateway"].Schema[key].Type {
			case schema.TypeString:
				diff.Attributes[key] = &terraform.ResourceAttrDiff{Old: d.Get(key).(string), New: "Medium"}
			case schema.TypeBool:
				diff.Attributes[key] = &terraform.ResourceAttrDiff{Old: strconv.FormatBool(d.Get(key).(bool)), New: strconv.FormatBool(true)}
			case schema.TypeMap:
				diff.Attributes["tags.%"] = &terraform.ResourceAttrDiff{Old: "0", New: "2"}
...
func TestAccAliCloudVpcIpv6Gateway_basic1897(t *testing.T) {
	...
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tf-testacc%svpcipv6gateway%d", defaultRegionToTest, rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudVpcIpv6GatewayBasicDependence1897)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
...
var AlicloudVpcIpv6GatewayMap1897 = map[string]string{}

func AlicloudVpcIpv6GatewayBasicDependence1897(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
