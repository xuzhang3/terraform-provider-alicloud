...
func TestUnitAliCloudGaAclEntryAttachment(t *testing.T) {
	...
	attributes := map[string]interface{}{
		"acl_id":            "CreateGaAclEntryAttachmentValue",
		"entry":             "CreateGaAclEntryAttachmentValue",
		"entry_description": "CreateGaAclEntryAttachmentValue",
	}
	...
	ReadMockResponse := map[string]interface{}{
		...
		"AclEntries": []interface{}{
			map[string]interface{}{
				"Entry":            "CreateGaAclEntryAttachmentValue",
				"EntryDescription": "CreateGaAclEntryAttachmentValue",
			},
		},
		"AclStatus": "active",
	}
	...
	notFoundResponseMock := func(errorCode string) (map[string]interface{}, error) {
		return nil, GetNotFoundErrorFromString(GetNotFoundMessage("alicloud_ga_acl_entry_attachment", errorCode))
	}
	...
	for index, errorCode := range errorCodes {
		...
		default:
			assert.Nil(t, err)
			dCompare, _ := schema.InternalMap(p["alicloud_ga_acl_entry_attachment"].Schema).Data(dInit.State(), nil)
			for key, value := range attributes {
	...
	diff, err := newInstanceDiff("alicloud_ga_acl_entry_attachment", attributes, attributesDiff, dInit.State())
	if err != nil {
		t.Error(err)
	}
	...
	for index, errorCode := range errorCodes {
		retryIndex := index - 1
		patches := gomonkey.ApplyMethod(reflect.TypeOf(&client.Client{}), "DoRequest", func(_ *client.Client, action *string, _ *string, _ *string, _ *string, _ *string, _ map[string]interface{}, _ map[string]interface{}, _ *util.RuntimeOptions) (map[string]interface{}, error) {
			if *action == "RemoveEntriesFromAcl" {
				switch errorCode {
				case "NonRetryableError":
					return failedResponseMock(errorCode)
				default:
