...
func resourceAliCloudEsaHttpIncomingResponseHeaderModificationRule() *schema.Resource {
	return &schema.Resource{
		Create: resourceAliCloudEsaHttpIncomingResponseHeaderModificationRuleCreate,
		Read:   resourceAliCloudEsaHttpIncomingResponseHeaderModificationRuleRead,
		Update: resourceAliCloudEsaHttpIncomingResponseHeaderModificationRuleUpdate,
		Delete: resourceAliCloudEsaHttpIncomingResponseHeaderModificationRuleDelete,
...
func resourceAliCloudEsaHttpIncomingResponseHeaderModificationRuleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alicloud_esa_http_incoming_response_header_modification_rule", action, AlibabaCloudSdkGoERROR)
	}

	d.SetId(fmt.Sprintf("%v:%v", request["SiteId"], response["ConfigId"]))

	return resourceAliCloudEsaHttpIncomingResponseHeaderModificationRuleRead(d, meta)
}

func resourceAliCloudEsaHttpIncomingResponseHeaderModificationRuleRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	esaServiceV2 := EsaServiceV2{client}

	objectRaw, err := esaServiceV2.DescribeEsaHttpIncomingResponseHeaderModificationRule(d.Id())
	if err != nil {
		if !d.IsNewResource() && NotFoundError(err) {
			log.Printf("[DEBUG] Resource alicloud_esa_http_incoming_response_header_modification_rule DescribeEsaHttpIncomingResponseHeaderModificationRule Failed!!! %s", err)
			d.SetId("")
			return nil
		}
		return WrapError(err)
...
func resourceAliCloudEsaHttpIncomingResponseHeaderModificationRuleUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	var request map[string]interface{}
	var response map[string]interface{}
...
func resourceAliCloudEsaHttpIncomingResponseHeaderModificationRuleDelete(d *schema.ResourceData, meta interface{}) error {
